...
struct Epoch2{S, T}
    epoch::Int64
    offset::T
    Epoch2{S}(epoch::Int64, offset::T) where {S, T} = new{S::TimeScale, T}(epoch, offset)
end

function Epoch2{S}(epoch::Int64, offset, Δt) where S
    sum = offset + Δt
...
function Epoch2{S}(date::Date, time::Time) where S
    seconds = second(time)
...
function Epoch2{S}(year::Int, month::Int, day::Int, hour::Int=0, minute::Int=0, second::Float64=0.0) where S
    Epoch2{S}(Date(year, month, day), Time(hour, minute, second))
end
...
tai_offset(ep::Epoch2{S}) where {S} = tai_offset(S, ep)

tai_offset(::InternationalAtomicTime, date, time) = 0.0

function tai_offset(scale, date, time)
    ref = Epoch2{TAI}(date, time)
    offset = 0.0
    for _ in 1:8
        offset = -tai_offset(scale, Epoch2{TAI}(ref, offset))
    end
    offset
end

function -(a::Epoch2, b::Epoch2)
    (a.epoch - b.epoch) + (a.offset - b.offset)
end

const JULIAN_EPOCH = Epoch2{TT}(AstroDates.JULIAN_EPOCH, AstroDates.H12)
const J2000_EPOCH = Epoch2{TT}(AstroDates.J2000_EPOCH, AstroDates.H12)
const EPOCH_77 = Epoch2{TAI}(1977, 1, 1)
